...
def start_attack_reply(message, target, port, time):
    ...
    response = f"{username}, \n✨✨Premium \nattack STARTED 🚀 BY TOXIC.\n\n📡 𝐓𝐚𝐫𝐠𝐞𝐭: {target}\n🔌 𝐏𝐨𝐫𝐭: {port}\n⏱️ 𝐓𝐢𝐦𝐞: {time} 𝐒𝐞𝐜𝐨𝐧𝐝𝐬\n\n𝐌𝐞𝐭𝐡𝐨𝐝: Premium \nBy https://t.me/TOXICPLAYER002"
    bot.reply_to(message, response)
...
def broadcast_message(message):
    ...
    if user_id in admin_id:
        ...
        if len(command) > 1:
            message_to_broadcast = "AGR MERI FILE SAHI SE WORK NHI KR RHI TI PAID LO 300 SEC OWNER @DDOS_SELLER_1:\n\n" + command[1]
            with open(USER_FILE, "r") as file:
                user_ids = file.read().splitlines()
                for user_id in user_ids:
                    try:
...
while True:
 ...
 #https://t.me/Kasukabe1 OWMER @offx_sahil